...
import { Camera, Layers, Zap, Copy, Download, ArrowLeft, CheckCircle2, Binary, X, FileCode, Star, Package, Tag, Users, UserCheck, Globe, Sun, Moon, Clipboard, CheckCircle, Info, ExternalLink, FileJson, Cloud, Search, Sparkles, FolderOpen } from 'lucide-react';
import { PACKS, EXTRAS, CANVAS_SIZES } from '../constants.js';
...
const CommandCenter = ({ graduates = [], staff = [], design = {}, groupName = "ORLA-GENERICA", course = "", group = "", onBack, theme, onToggleTheme, settings = {} }) => {
    ...
    showSuccess("${customMsg}");`;
            ...
            if (type === 'CONSTRUCTOR') {
                filename = `01_ORLA_CONSTRUCTOR_V4_8_${timestamp}.jsx`;
                console.log(">>> GENERANDO CONSTRUCTOR V4.8 - " + timestamp);
            const staffInfo = (() => {
                const totalDocentes = (currentStaff || []).length;
                if (totalDocentes === 0) return { lines: '', maxDocBaseline: Number(design.dY || 0), allRoles: [] };
                
                const aW = Number(design.aW) || 350;
                const aH = Number(design.aH) || 450;
                const dScale = Number(design.dScale) || 1.1; 
                const dynamicW = Math.round(aW * dScale);
                const dynamicH = Math.round(aH * dScale);
                const dGapX = Number(design.dGapX ?? 60); 
                const dGapY = Number(design.dGapY ?? 100); 
                const canvasW = Number(design.canvasW) || 4961;
                
                const dY_start = Number(design.dY || 0) - 177.17; // Sube TODO (docentes y alumnos) 1.5cm 
                const dOffsetX = Number(design.dOffsetX) || 0;
                const dTextOffset = Number(design.aTextOffset ?? 0);

                const aScale = Number(design.aScale) || 1.0;
                const fSizeName = Math.round((Number(design.fontSizeAluName || 36) * aScale));
                const fSizeSur = Math.round((Number(design.fontSizeAluSur || 28) * aScale));
                const fSizeRole = Math.round((Number(design.fontSizeAluSur || 28) * aScale));

                const staffCols = totalDocentes > 10 ? 10 : totalDocentes;
                const staffRows = [];
                for (let i = 0; i < (currentStaff || []).length; i += staffCols) {
                    staffRows.push((currentStaff || []).slice(i, i + staffCols));
                }

                let result = [];
                let maxDocBaseline = dY_start;
                const allRoles = (currentStaff || []).map(s => s.role || s.assignment || "").filter(r => r !== "");

                staffRows.forEach((rowItems, rowIndex) => {
                    const rowW = (rowItems.length * dynamicW) + ((rowItems.length - 1) * dGapX);
                    const rowStartX = ((canvasW - rowW) / 2) + dOffsetX;
                    const phY_row = dY_start + (rowIndex * (dynamicH + dGapY));

                    rowItems.forEach((member, i) => {
                        const phX = rowStartX + (i * (dynamicW + dGapX));
                        const url = buildUrl(member);
                        const nameParts = splitName(member.name || "Docente");
                        ...
                        const nameBaseline = phY_row + dynamicH + dTextOffset + (fSizeName * 0.9);
                        const surBaseline = (nameParts.apellidos && !design.hideApellidosDoc) ? (nameBaseline + (fSizeName * 1.15)) : 0;
                        const roleBaseline = (member.role || member.assignment) && !design.hideCargoDoc ? ((surBaseline > 0 ? surBaseline : nameBaseline) + (fSizeSur * 1.15)) : 0;

                        const currentBottom = roleBaseline > 0 ? roleBaseline : (surBaseline > 0 ? surBaseline : nameBaseline);
                        if (currentBottom > maxDocBaseline) maxDocBaseline = currentBottom;

                        result.push(`    createItem(docentesGroup, ${esc(nameParts.nombre)}, ${esc(nameParts.apellidos)}, ${esc(member.role || member.assignment || "")}, ${esc(photoId)}, ${phX.toFixed(2)}, ${phY_row.toFixed(2)}, ${dynamicW.toFixed(2)}, ${dynamicH.toFixed(2)}, ${nameBaseline.toFixed(2)}, ${fSizeName}, ${surBaseline.toFixed(2)}, ${fSizeSur}, ${roleBaseline.toFixed(2)}, ${fSizeRole}, true, ${esc(url)}, ${zoom}, ${offX_p}, ${offY_p});`);
                    });
                });
                return { lines: result.join('\n'), maxDocBaseline, allRoles };
            })();
            ...
            const aluLines = (() => {
                ...
                const aH = Number(design.aH) || 450;
                const aScale = Number(design.aScale) || 1.0;
                const dynamicW = Math.round(aW * aScale);
                const dynamicH = Math.round(aH * aScale);
                const aGapX = Number(design.aGapX || 0);
                const aGapY = Number(design.aGapY || 118.11); // 1cm exacto por defecto
                const canvasW = Number(design.canvasW) || 4961;
                const aCols = Number(design.aCols || 8);
                const aStartY = staffInfo.maxDocBaseline + 118.11; // 1 cm EXACTO debajo del texto de docentes
                const aOffsetX = Number(design.aOffsetX) || 0;
                const fSizeName = Math.round((Number(design.fontSizeAluName || 36) * aScale)); 
                const fSizeSur = Math.round((Number(design.fontSizeAluSur || 28) * aScale));
                const aTextOffset = Number(design.aTextOffset ?? 0);
                ...
                const orderedRows = distributeOrders(sortedAlus, aCols, design.studentRowBalance);
                let result = [];
                let currentY = aStartY;

                orderedRows.forEach((rowItems, rowIndex) => {
                    const rowWidth = (rowItems.length * dynamicW) + ((rowItems.length - 1) * aGapX);
                    const rowStartX = (canvasW / 2) - (rowWidth / 2) + aOffsetX;
                    
                    const phY = currentY;
                    let maxRowTextBottom = phY + dynamicH; // Mínimo es el final de la foto

                    rowItems.forEach((g, colIndex) => {
                        const phX = rowStartX + (colIndex * (dynamicW + aGapX));
                        ...
                        const nameBaseline = phY + dynamicH + aTextOffset + (fSizeName * 0.9);
                        const surBaseline = (nameParts.apellidos && !design.hideApellidosAlu) ? (nameBaseline + (fSizeName * 1.15)) : 0;
                        
                        const itemBottom = surBaseline > 0 ? surBaseline : nameBaseline;
                        if (itemBottom > maxRowTextBottom) maxRowTextBottom = itemBottom;

                        result.push(`    createItem(alumnosGroup, ${esc(nameParts.nombre)}, ${esc(nameParts.apellidos)}, "", ${esc(photoId)}, ${phX.toFixed(2)}, ${phY.toFixed(2)}, ${dynamicW.toFixed(2)}, ${dynamicH.toFixed(2)}, ${nameBaseline.toFixed(2)}, ${fSizeName}, ${surBaseline.toFixed(2)}, ${fSizeSur}, 0, 0, false, ${esc(url)}, ${zoom}, ${offX_p}, ${offY_p});`);
                    });

                    // La siguiente fila empieza 1cm (118.11 px) debajo del texto más bajo de esta fila
                    currentY = maxRowTextBottom + 118.11;
                });
                return result.join('\n');
            })();
            ...
            content = [
                ...
                '    $.writeln(">>> INICIANDO MOTOR V4.0 PREMIUM <<<");',
                ...
                '    var staffSurSizeMaster = 0; // Para uniformizar apellidos de docentes\n    var staffRoleSizeMaster = 0; // Para uniformizar cargos de docentes\n    var allStaffRoles = ' + JSON.stringify(staffInfo.allRoles) + ';',
                '    ',
                '    try {',
                ...
                '            try {',
                '                if (!layer || layer.kind !== LayerKind.TEXT) return 0;',
                '                doc.activeLayer = layer; // Fuerza actualización de bounds',
                '                var currentW = layer.bounds[2] - layer.bounds[0];',
                '                if (currentW > maxW && currentW > 0) {',
                '                    var ratio = maxW / currentW;',
                '                    var minSize = 5 * PX_TO_PT;',
                '                    layer.textItem.size = Math.max(minSize, layer.textItem.size * ratio);',
                '                }',
                '                return layer.textItem.size;',
                '            } catch (e) { return 0; }',
                '        }',
                '',
                `        var currentShape = ${JSON.stringify(design.photoShape || 'circle')};`,
                `        var defaultFont = ${JSON.stringify(design.fontFamily || "MyriadPro-Bold")};`,
                '        var PX_TO_PT = 72 / (doc.resolution || 300);',
                '',
                '        // PRE-CALIBRACI\u00D3N CARGOS DOCENTES: Encontrar el m\u00CDnimo común divisor',
                '        if (allStaffRoles && allStaffRoles.length > 0) {',
                '            var tempRoleL = doc.artLayers.add(); tempRoleL.kind = LayerKind.TEXT;',
                '            tempRoleL.textItem.kind = TextType.POINTTEXT;',
                '            applyFontRobust(tempRoleL.textItem, defaultFont.replace("-Bold", "-Regular"));',
                '            var minR = 999; var phW_ref = ' + (Number(design.aW || 350) * Number(design.dScale || 1.1)) + ';',
                '            for (var r=0; r<allStaffRoles.length; r++) {',
                '                if (!allStaffRoles[r]) continue;',
                '                tempRoleL.textItem.size = 28 * PX_TO_PT; ',
                '                tempRoleL.textItem.contents = String(allStaffRoles[r]).toUpperCase();',
                '                var fSizeFound = autoFitFontSize(tempRoleL, phW_ref * 0.85);',
                '                if (fSizeFound < minR && fSizeFound > 0) minR = fSizeFound;',
                '            }',
                '            if (minR < 999) staffRoleSizeMaster = minR;',
                '            tempRoleL.remove();',
                '        }',
                '',
                ...
                '',
            '        // HELPER: Creación de cada item (Foto + Textos)',
            '        function createItem(group, nombre, apellidos, cargo, id, phX, phY, phW, phH, nameBaseline, nameSizePx, surBaseline, surSizePx, roleBaseline, roleSizePx, isStaff, photoUrl, zoom, extraX, extraY) {',
            '            try {',
            '                var itemGroup = group.layerSets.add();',
            '                itemGroup.name = id + "_" + nombre;',
            '                ',
            '                // M\u00C1SCARA / FORMA',
            `                var currentShape = ${JSON.stringify(design.photoShape || "circle")};`,
            '                var sW = phW; var sH = phH;',
            '                if (currentShape === "circle" || currentShape === "square") {',
            '                    var minS = Math.min(phW, phH);',
            '                    sW = minS; sH = minS;',
            '                }',
            ...
            '                }',
            '',
            '                var placeholder = itemGroup.artLayers.add(); placeholder.name = "PLACEHOLDER";',
            ...
            '                            var shiftX = (extraX / 100) * (newB[2] - newB[0]);',
            '                            var shiftY = (extraY / 100) * (newB[3] - newB[1]);',
            '                            pastedLayer.translate(pb[0] + (pb[2]-pb[0])/2 - (newB[0] + (newB[2]-newB[0])/2) + shiftX, pb[1] + (pb[3]-pb[1])/2 - (newB[1] + (newB[3]-newB[1])/2) + shiftY);',
            ...
            '                ',
            '                // NOMBRE (Línea 1)',
            '                var nameLayer = itemGroup.artLayers.add(); nameLayer.kind = LayerKind.TEXT;',
            ...
            '                nameLayer.textItem.contents = nombre.toUpperCase();',
                '                nameLayer.textItem.position = [centerX, nameBaseline];',
                '                // L\u00CDMITE DE NOMBRE: Se reduce para staff para que coincida con el tamaño visual de los alumnos',
                '                autoFitFontSize(nameLayer, phW * 1.1);',
                '',
                '                // APELLIDOS (L\u00CDnea 2)',
                '                if (apellidos && surBaseline > 0) {',
                '                    var surLayer = itemGroup.artLayers.add(); surLayer.kind = LayerKind.TEXT;',
                ...
                '                    surLayer.textItem.size = surSizePx * PX_TO_PT;',
                '                    surLayer.textItem.contents = apellidos.toUpperCase();',
                '                    surLayer.textItem.position = [centerX, surBaseline];',
                '                    // UNIFORMIDAD DOCENTES: Todos como el primero',
                '                    if (isStaff) {',
                '                        if (staffSurSizeMaster === 0) {',
                '                            staffSurSizeMaster = autoFitFontSize(surLayer, phW * 1.1);',
                '                        } else {',
                '                            surLayer.textItem.size = staffSurSizeMaster;',
                '                        }',
                '                    } else {',
                '                        autoFitFontSize(surLayer, phW * 1.1);',
                '                    }',
                '                }',
                '',
                '                // CARGO (L\u00CDnea 3 - Solo Docentes)',
                '                if (isStaff && cargo && roleBaseline > 0) {',
                '                    var roleLayer = itemGroup.artLayers.add(); roleLayer.kind = LayerKind.TEXT;',
                '                    roleLayer.textItem.kind = TextType.POINTTEXT;',
                '                    roleLayer.textItem.justification = Justification.CENTER;',
                '                    applyFontRobust(roleLayer.textItem, defaultFont.replace("-Bold", "-Regular"));',
                '                    roleLayer.textItem.size = (staffRoleSizeMaster > 0) ? staffRoleSizeMaster : (roleSizePx * PX_TO_PT);',
                '                    roleLayer.textItem.contents = cargo.toUpperCase();',
                '                    roleLayer.textItem.position = [centerX, roleBaseline];',
                '                    if (staffRoleSizeMaster === 0) autoFitFontSize(roleLayer, phW * 0.85);',
                '                }',
            '                ',
            ...
            '        }',
                ...
                '',
                '        // ---------------------------------------------------------',
                '        // CONTENIDOS DINÁMICOS (Inyectados desde el servidor)',
                '        // ---------------------------------------------------------',
                staffInfo.lines,
                aluLines,
                '',
                '        // ---------------------------------------------------------',
                '        // PIE DE ORLA (Clon del motor CSS del editor)',
                '        // ---------------------------------------------------------',
                '        var cX = doc.width / 2;',
                `        var docH = Number(${design.canvasH || 3508});`,
                `        var margin = Number(${design.margin || 20});`,
                `        var footerY = Number(${design.footerY || 0});`,
                '        // Punto de anclaje inferior (como el \'bottom\' de CSS)',
                '        var footerAnchorY = docH - (margin + footerY);',
                '',
                '        // Los textos del pie se añaden de abajo hacia arriba para evitar solapes',
                '        // 1. Curso (Bottom)',
                ...
                `            var cSize = Number(${design.fontSizeCourse || 35});`,
                '            cL.textItem.size = cSize * PX_TO_PT;',
                '            cL.textItem.tracking = 150;',
                '            cL.textItem.justification = Justification.CENTER;',
                '            applyFontRobust(cL.textItem, defaultFont);',
                '            cL.textItem.position = [cX, footerAnchorY];',
                '            footerAnchorY -= (cSize * 1.1); // Subimos el anclaje para el siguiente',
                '        }',
                '',
                '        // 2. Promoción (Middle)',
                `        if (!${!!design.hidePromo}) {`,
                ...
                `            var pSize = Number(${design.fontSizePromo || 45});`,
                '            pL.textItem.size = pSize * PX_TO_PT;',
                '            pL.textItem.tracking = 600;',
                '            pL.textItem.justification = Justification.CENTER;',
                '            applyFontRobust(pL.textItem, defaultFont);',
                '            if (typeof cL !== "undefined" && cL) footerAnchorY -= 20;',
                '            pL.textItem.position = [cX, footerAnchorY];',
                '            footerAnchorY -= (pSize * 1.3);',
                '        }',
                '',
                '        // 3. Colegio (Top)',
                ...
                `            var sSize = Number(${design.fontSizeSchool || 120});`,
                '            sL.textItem.size = sSize * PX_TO_PT;',
                '            sL.textItem.justification = Justification.CENTER;',
                '            applyFontRobust(sL.textItem, defaultFont);',
                '            sL.textItem.position = [cX, footerAnchorY];',
                '        }',
                '',
    ...
    showSuccess("\u00A1INYECCI\u00D3N CLOUD COMPLETADA!");`,
        ...
        // 1. Intentar Guardado y Apertura en Local (Solo dev con API local) - PERMITE REVELAR EN FINDER
            ...
            try {
                ...
                if (r.ok) {
                    ...
                    if (data.success) {
                        ...
                        return; // Salir si ya se guardó con éxito
                    } else if (data.cancelled) {
                        return; // El usuario canceló el diálogo de guardado
                    }
                }
            } catch (e) { console.error("Error local save API:", e); }
        }

        // 2. Método Navegador Moderno: File System Access API (Elegir ubicación)
        if (!alreadySaved && 'showSaveFilePicker' in window) {
            try {
                ...
                setScriptModal({ 
                    ...
                    copied: false, 
                    saved: true, 
                    savedPath: 'Ubicación seleccionada por usuario',
                    method: 'picker' 
                });
                return;
            } catch (e) {
                console.log("File System Access API omitida:", e);
                if (e.name === 'AbortError') return; // Usuario canceló
            }
        }

        // 3. Método Universal: Descarga por bloque de datos (Tradicional)
        try {
            ...
            setScriptModal({ 
                ...
                copied: false, 
                saved: true, 
                savedPath: 'Carpeta de Descargas (Navegador)',
                method: 'download'
            });
        } catch (error) {
                        ...
                        <div className="space-y-4 md:space-y-2 w-full">
                            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                ...
                                <div className="flex items-center gap-2">
                                    <span className="text-[9px] md:text-[10px] bg-violet-500/20 text-violet-400 px-3 py-1.5 md:py-1 rounded-full border border-violet-500/30 tracking-widest font-black uppercase inline-block">V4.8 READY</span>
                                    <button
                        ...
                        <div className={`group ${theme === 'dark' ? 'bg-white/[0.03] border-white/10' : 'bg-white border-slate-200 shadow-sm'} border rounded-[32px] p-6 md:p-8 space-y-4 hover:border-blue-500/30 transition-all duration-500 flex flex-col`}>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 py-4">
                                <div className={`${theme === 'dark' ? 'bg-blue-500/5 border-blue-500/10' : 'bg-blue-50 border-blue-100'} rounded-2xl border p-5 flex flex-col justify-center`}>
                                    <p className={`text-[8px] uppercase font-black ${theme === 'dark' ? 'text-white/30' : 'text-blue-900/40'} tracking-widest leading-none mb-1`}>TAMAÑO</p>
                                    <p className={`text-sm font-black italic uppercase ${theme === 'dark' ? 'text-white' : 'text-blue-900'}`}>
                                        {CANVAS_SIZES.find(s => s.id === design.canvasSize)?.id || "PERSONALIZADO"}
                                    </p>
                                </div>
                                <div className={`${theme === 'dark' ? 'bg-blue-500/5 border-blue-500/10' : 'bg-blue-50 border-blue-100'} rounded-2xl border p-5 flex flex-col justify-center`}>
                                    <p className={`text-[8px] uppercase font-black ${theme === 'dark' ? 'text-white/30' : 'text-blue-900/40'} tracking-widest leading-none mb-1`}>LIENZO</p>
                                    <p className={`text-sm font-black italic ${theme === 'dark' ? 'text-white' : 'text-blue-900'}`}>
                                        {CANVAS_SIZES.find(s => s.id === design.canvasSize)?.label || 
                                         `${(design.canvasW * 25.4 / (design.dpi || 300) / 10).toFixed(1)} x ${(design.canvasH * 25.4 / (design.dpi || 300) / 10).toFixed(1)} CM`}
                                    </p>
                                </div>
                                <div className={`${theme === 'dark' ? 'bg-blue-500/5 border-blue-500/10' : 'bg-blue-50 border-blue-100'} rounded-2xl border p-5 flex flex-col justify-center`}>
                                    <p className={`text-[8px] uppercase font-black ${theme === 'dark' ? 'text-white/30' : 'text-blue-900/40'} tracking-widest leading-none mb-1`}>MARGEN GLOBAL</p>
                                    <p className={`text-sm font-black italic ${theme === 'dark' ? 'text-white' : 'text-blue-900'}`}>5 MM</p>
                                </div>
                                <div className={`${theme === 'dark' ? 'bg-blue-500/5 border-blue-500/10' : 'bg-blue-50 border-blue-100'} rounded-2xl border p-5 flex flex-col justify-center`}>
                                    <p className={`text-[8px] uppercase font-black tracking-widest ${theme === 'dark' ? 'text-blue-400' : 'text-blue-600'}`}>MOTOR</p>
                                    <p className={`text-sm font-black italic ${theme === 'dark' ? 'text-white' : 'text-blue-900'}`}>Pujalte Engine 4.8</p>
                                </div>
                            </div>
                            ...
                            <button onClick={() => downloadScript('CONSTRUCTOR')} className="w-full flex items-center justify-between p-6 bg-blue-600/10 hover:bg-blue-600 hover:text-white text-blue-400 rounded-3xl border border-blue-500/30 transition-all shadow-xl active:scale-95 group/btn">
                                <div className="flex items-center gap-5">
                                    ...
                                    <div className="text-left">
                                        <div className="font-black text-xl tracking-tight">CONSTRUCTOR V4.8</div>
                    ...
                    <div className={`${theme === 'dark' ? 'bg-[#0a0a0a] border-white/10' : 'bg-white border-slate-200'} border rounded-[40px] w-full max-w-sm shadow-2xl overflow-hidden animate-scale-in`} onClick={e => e.stopPropagation()}>
                        <div className="p-8 text-center space-y-6">
                            ...
                            {scriptModal.saved ? (
                                <div className={`space-y-1 py-4 px-6 ${theme === 'dark' ? 'bg-emerald-500/5 border-emerald-500/20' : 'bg-emerald-50 border-emerald-100'} border rounded-3xl`}>
                                    ...
                                    <p className="text-emerald-500 text-[10px] font-black uppercase tracking-widest">
                                        {scriptModal.method === 'picker' ? '¡UBICACIÓN ELEGIDA!' : 
                                         scriptModal.method === 'local_api' ? '¡GUARDADO EN EL ACTO!' : 
                                         'ARCHIVO DESCARGADO'}
                                    </p>
                                    <p className={`${theme === 'dark' ? 'text-emerald-300/40' : 'text-emerald-700/50'} text-[9px] font-mono italic break-all line-clamp-2`}>
                                        {scriptModal.savedPath}
                                    </p>
                                </div>
                            ) : (
                            ...
                            <div className="space-y-4 py-2">
                                ...
                                <div className="space-y-3">
                                    {[
                                        scriptModal.method === 'picker' ? 'El archivo ya está donde elegiste' : 'Localiza el archivo generado',
                            ...
                            <div className="space-y-2 pt-2">
                                {scriptModal.method === 'local_api' && (
                                    <button
                                        ...
                                        className="w-full py-4 bg-emerald-600 hover:bg-emerald-500 text-white rounded-2xl font-black text-[11px] uppercase tracking-[0.2em] transition-all active:scale-95 shadow-lg shadow-emerald-900/30 flex items-center justify-center gap-2"
                                    >
                                        <FolderOpen size={16} />
                                        <span>Revelar en Carpeta</span>
                                    </button>
                                )}
