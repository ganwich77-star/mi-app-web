...
import {
    ...
    Settings2, Maximize, LayoutGrid, UserCheck, Eye, Code, Terminal, Type, Bold, Italic, AlignLeft,
    Move, Layout, MousePointer2, ZoomIn, ZoomOut
} from 'lucide-react';
...
export default function App() {
    ...
    const [configOrla, setConfigOrla] = useState({
        ...
        fontFamily: "Montserrat", isBold: true, isItalic: false,
        fontSizeAlu: 10, fontSizeDoc: 13,
        dScale: 1.2, dY: mmToPx(60), dTextOffset: mmToPx(12),
        aW: mmToPx(35), aH: mmToPx(45), aStartY: mmToPx(135),
        aCols: 8, aGapY: mmToPx(65), aTextOffset: mmToPx(10)
    });
    ...
    // 3. Sistema de Seguridad (Anti-Copia y Anti-Debug)
    useEffect(() => {
        // Bloquear clic derecho
        const handleContextMenu = (e) => {
            if (isAdminUnlocked) return; // Permitir en admin si está desbloqueado
            e.preventDefault();
        };

        // Bloquear selección de texto (CSS fallback en App.css también recomendado)
        ...
        const handleKeyDown = (e) => {
            ...
            // F12, Ctrl+Shift+I/J, Ctrl+U (Ver código fuente)
            ...
            if (isInspect) {
                e.preventDefault();
                alert('Protección de Propiedad Intelectual Activa - Pujalte Studio');
            }
        };
    ...
    const renderDesignControls = () => {
        ...
        const ControlGroup = ({ title, icon: Icon, children }) => (
            <div className="space-y-2">
                <h4 className="text-[10px] font-black text-violet-400 uppercase tracking-[0.2em] flex items-center gap-2 mb-2">
                    <Icon size={14} /> {title}
                </h4>
        ...
        const HybridInput = ({ label, value, onChange, min, max, step = 1, unit = "px" }) => (
            <div className="group space-y-1">
                <div className="flex justify-between items-center gap-2">
                    <label className={`text-[10px] font-black leading-tight transition-colors uppercase tracking-wider ${isFullScreenDesign ? 'text-white/40 group-hover:text-white/60' : 'text-primary/50 group-hover:text-primary/70'}`}>{label}</label>
                    <div className="flex items-center gap-2 shrink-0">
                        <input
                            type="number"
                            value={unit === "px" ? pxToMm(value) : value}
                            onChange={(e) => {
                                let val = parseFloat(e.target.value) || 0;
                                onChange(unit === "px" ? mmToPx(val) : val);
                            }}
                            className={`w-14 rounded-lg py-1 px-1 text-center text-xs font-bold outline-none focus:border-violet-500/50 transition-all ${isFullScreenDesign ? 'bg-white/5 border border-white/10 focus:bg-white/10 text-white' : 'bg-primary/5 border border-primary/10 focus:bg-primary/10 text-violet-500'}`}
                        />
                        <span className={`text-[9px] font-bold uppercase w-6 shrink-0 ${isFullScreenDesign ? 'text-white/40' : 'text-primary/30'}`}>{unit === "px" ? "mm" : unit}</span>
                    </div>
                </div>
                <input
                    type="range" min={min} max={max} step={step} value={value}
                    onChange={(e) => onChange(parseFloat(e.target.value))}
                    className={`w-full h-1.5 rounded-lg appearance-none cursor-pointer accent-violet-500 hover:accent-violet-400 transition-all ${isFullScreenDesign ? 'bg-white/10' : 'bg-primary/10'}`}
                />
            </div>
        );
        ...
        return (
            <div className="space-y-4">
                <ControlGroup title="Tipografía y Textos" icon={Type}>
                    <div className="space-y-4">
                        <div className="space-y-2">
                            <label className={`text-[10px] font-black uppercase tracking-widest block ${isFullScreenDesign ? 'text-white/40' : 'text-primary/50'}`}>Fuente Base</label>
                            <div className="relative">
                                <select
                                    value={configOrla.fontFamily}
                                    onChange={(e) => updateConfig('fontFamily', e.target.value)}
                                    className={`w-full rounded-xl px-4 text-xs font-black outline-none focus:border-violet-500/50 cursor-pointer appearance-none transition-all py-2.5 ${isFullScreenDesign ? 'bg-white/5 border border-white/10 text-white hover:bg-white/10' : 'bg-primary/5 border border-primary/10 text-primary hover:bg-primary/10'}`}
                                >
                                    {['Montserrat', 'Inter', 'Outfit', 'Playfair Display', 'Roboto Mono'].map(f => (
                                        <option key={f} value={f} className={isFullScreenDesign ? "bg-slate-900" : ""}>{f}</option>
                                    ))}
                                </select>
                                <ChevronDown size={16} className={`absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none ${isFullScreenDesign ? 'text-white/20' : 'text-primary/30'}`} />
                            </div>
                        </div>

                        <div className="flex gap-2">
                            <button
                                onClick={() => updateConfig('isBold', !configOrla.isBold)}
                                className={`flex-1 rounded-xl text-[10px] font-black uppercase tracking-widest border transition-all py-2.5 ${configOrla.isBold ? 'bg-violet-600 border-violet-500 text-white shadow-lg shadow-violet-600/20' : isFullScreenDesign ? 'bg-white/5 border-white/10 text-white/40 hover:bg-white/10' : 'bg-primary/5 border-primary/10 text-primary/60 hover:bg-primary/10'}`}
                            >
                                <Bold size={14} className="inline mr-2" /> Negrita
                            </button>
                            <button
                                onClick={() => updateConfig('isItalic', !configOrla.isItalic)}
                                className={`flex-1 rounded-xl text-[10px] font-black uppercase tracking-widest border transition-all py-2.5 ${configOrla.isItalic ? 'bg-violet-600 border-violet-500 text-white shadow-lg shadow-violet-600/20' : isFullScreenDesign ? 'bg-white/5 border-white/10 text-white/40 hover:bg-white/10' : 'bg-primary/5 border-primary/10 text-primary/60 hover:bg-primary/10'}`}
                            >
                                <Italic size={14} className="inline mr-2" /> Cursiva
                            </button>
                        </div>

                        <HybridInput
                            label="Tamaño fuente alumnos"
                            value={configOrla.fontSizeAlu}
                            onChange={(val) => updateConfig('fontSizeAlu', val)}
                            min={6} max={25} step={0.5} unit="pt"
                        />
                        <HybridInput
                            label="Tamaño fuente docentes"
                            value={configOrla.fontSizeDoc}
                            onChange={(val) => updateConfig('fontSizeDoc', val)}
                            min={8} max={35} step={0.5} unit="pt"
                        />
                    </div>
                </ControlGroup>

                <ControlGroup title="Estructura y Fotos" icon={Layout}>
                    <div className="space-y-3">
                        <HybridInput
                            label="Tamaño de Contenedor"
                            value={configOrla.aW}
                            onChange={updatePhotoSize}
                            min={mmToPx(25)} max={mmToPx(55)} unit="px"
                        />
                        <div className="grid grid-cols-2 gap-4">
                            <HybridInput
                                label="Columnas G."
                                value={configOrla.aCols}
                                onChange={(val) => updateConfig('aCols', val)}
                                min={4} max={12} step={1} unit="ud"
                            />
                            <HybridInput
                                label="Espacio Filas"
                                value={configOrla.aGapY}
                                onChange={(val) => updateConfig('aGapY', val)}
                                min={mmToPx(40)} max={mmToPx(100)} unit="px"
                            />
                        </div>
                        <HybridInput
                            label="Margen Seguridad"
                            value={configOrla.margin}
                            onChange={(val) => updateConfig('margin', val)}
                            min={mmToPx(5)} max={mmToPx(40)} unit="px"
                        />
                    </div>
                </ControlGroup>

                <ControlGroup title="Posicionamiento" icon={Move}>
                    <div className="space-y-3">
                        <HybridInput
                            label="Escala Docentes"
                            value={configOrla.dScale}
                            onChange={(val) => updateConfig('dScale', val)}
                            min={0.8} max={1.8} step={0.05} unit="x"
                        />
                        <HybridInput
                            label="Altura Docentes (Y)"
                            value={configOrla.dY}
                            onChange={(val) => updateConfig('dY', val)}
                            min={mmToPx(20)} max={mmToPx(130)} unit="px"
                        />
                        <HybridInput
                            label="Altura Alumnos (Y)"
                            value={configOrla.aStartY}
                            onChange={(val) => updateConfig('aStartY', val)}
                            min={mmToPx(80)} max={mmToPx(220)} unit="px"
                        />
                    </div>
                </ControlGroup>
            </div>
    ...
    const downloadConfigCSV = () => {
        const headers = "parametro,valor,unidad,desc\n";
        const rows = [
            `canvas_w,${configOrla.canvasW},px,Ancho A3 300dpi`,
            `canvas_h,${configOrla.canvasH},px,Alto A3 300dpi`,
            `margin,${configOrla.margin},px,Margen de seguridad`,
            `alu_cols,${configOrla.aCols},cnt,Columnas grid alumnos`,
            `alu_w,${configOrla.aW},px,Ancho foto alumno`,
            `alu_h,${configOrla.aH},px,Alto foto alumno`,
            `alu_gap_y,${configOrla.aGapY},px,Salto entre filas`,
            `alu_start_y,${configOrla.aStartY},px,Inicio primer fila`,
            `font_family,${configOrla.fontFamily},str,Fuente tipográfica`,
            `font_bold,${configOrla.isBold ? 1 : 0},bin,Peso negrita`,
            `font_size_alu,${configOrla.fontSizeAlu},pt,Tamaño texto alumnos`
        ].join("\n");
        const blob = new Blob([headers + rows], { type: 'text/csv' });
        const url = window.URL.createObjectURL(blob);
        const a = document.createElement('a'); a.href = url; a.download = `config_diseno_${schools.find(s => s.id === adminSchool)?.code || 'orla'}.csv`;
        a.click();
    };
    ...
    return (
        <div className="min-h-screen transition-colors duration-500">
            ...
            {view !== 'master' && view !== 'onboarding' && view !== 'landing' && !settings.isSuspended && (
                <>
                    <header className={`fixed top-0 inset-x-0 backdrop-blur-xl border-b safe-top transition-all duration-500 ${isFullScreenDesign ? 'z-[700] bg-slate-950 border-white/5' : 'z-50 bg-main/80 border-primary/5'}`}>
                        <div className={`${isFullScreenDesign ? 'w-full pl-6 pr-0' : (view === 'admin' ? 'w-full px-6' : 'max-w-lg px-6')} mx-auto h-20 flex items-center justify-between transition-all duration-500`}>
                            <button onClick={handleAdminClick} className="flex items-center gap-3 active:scale-95 transition-transform group">
                                <div className="text-left">
                                    <h1 className={`text-xs font-black tracking-widest leading-none uppercase transition-colors ${isFullScreenDesign ? 'text-white/80 group-hover:text-white' : 'text-primary/80 group-hover:text-primary'}`}>Powered by</h1>
                                    <p className={`text-[9px] uppercase font-black tracking-[0.2em] ${isFullScreenDesign ? 'text-white/40' : 'text-primary/40'}`}>Creative Studio</p>
                                </div>
                            </button>
                            <div className={`flex items-center ${isFullScreenDesign ? 'h-full' : 'gap-3'}`}>
                                {view === 'admin' ? (
                                    isFullScreenDesign ? (
                                        <button
                                            onClick={() => setIsFullScreenDesign(false)}
                                            className="flex items-center justify-center gap-2 h-full w-[300px] border-l border-white/5 bg-slate-900 text-white/50 hover:text-white text-[10px] font-black uppercase tracking-widest transition-colors"
                                        >
                                            <ChevronRight size={14} className="rotate-180" /> Volver al Panel Normal
                                        </button>
                                    ) : (
                                        <button
                                            onClick={() => { setView('user'); setStep(1); }}
                                            className="flex items-center gap-2 px-4 py-2 bg-accent/10 border border-accent/20 text-accent text-[10px] font-black uppercase tracking-widest hover:bg-accent/20 transition-all rounded-xl"
                                        >
                                            <ChevronRight size={14} className="rotate-180" /> Volver Atrás
                                        </button>
                                    )
                                ) : (
                    ...
                    <div className="pt-0 pb-8 min-h-screen animate-fade-in">
                        <div className="max-w-5xl mx-auto px-4">
                            ...
                            <div className="w-full mb-8">
                                <div className="w-full flex flex-wrap md:flex-nowrap gap-1.5 bg-primary/5 p-1.5 rounded-2xl border border-primary/10 backdrop-blur-md">
                                    <button onClick={() => { setAdminTab('shooting'); setShootSearch(''); }} className={`flex-1 px-4 py-3 rounded-xl text-[11px] font-black uppercase tracking-wider transition-all ${adminTab === 'shooting' ? 'bg-red-700 text-white shadow-lg shadow-red-700/20' : 'text-secondary hover:text-primary hover:bg-primary/5'}`}>📸 Shooting</button>
                                    <button onClick={() => setAdminTab('schools')} className={`flex-1 px-4 py-3 rounded-xl text-[11px] font-black uppercase tracking-wider transition-all ${adminTab === 'schools' ? 'bg-orange-500 text-white shadow-lg shadow-orange-500/20' : 'text-secondary hover:text-primary hover:bg-primary/5'}`}><GraduationCap size={14} className="inline md:mr-2" /> <span className="hidden md:inline">Centros</span></button>
                                    <button onClick={() => setAdminTab('settings')} className={`flex-1 px-4 py-3 rounded-xl text-[11px] font-black uppercase tracking-wider transition-all ${adminTab === 'settings' ? 'bg-indigo-500 text-white shadow-lg shadow-indigo-500/20' : 'text-secondary hover:text-primary hover:bg-primary/5'}`}><Settings size={14} className="inline md:mr-2" /> <span className="hidden md:inline">Ajustes App</span></button>
                                    <button onClick={() => setAdminTab('orders')} className={`flex-1 px-4 py-3 rounded-xl text-[11px] font-black uppercase tracking-wider transition-all ${adminTab === 'orders' ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20' : 'text-secondary hover:text-primary hover:bg-primary/5'}`}><Users size={14} className="inline md:mr-2" /> <span className="hidden lg:inline">Gestión Pedidos</span></button>
                                    <button onClick={() => setAdminTab('precios')} className={`flex-1 px-4 py-3 rounded-xl text-[11px] font-black uppercase tracking-wider transition-all ${adminTab === 'precios' ? 'bg-amber-500 text-white shadow-lg shadow-amber-500/20' : 'text-secondary hover:text-primary hover:bg-primary/5'}`}><Euro size={14} className="inline md:mr-2" /> <span className="hidden md:inline">Precios</span></button>
                                    <button onClick={() => setAdminTab('design')} className={`flex-1 px-4 py-3 rounded-xl text-[11px] font-black uppercase tracking-wider transition-all ${adminTab === 'design' ? 'bg-violet-600 text-white shadow-lg shadow-violet-600/20' : 'text-secondary hover:text-primary hover:bg-primary/5'}`}><LayoutGrid size={14} className="inline md:mr-2" /> <span className="hidden md:inline">Diseño Orla</span></button>
                                </div>
                            </div>
                            ...
                            {/* ── SECCIÓN DISEÑO ORLA ────────────────────────── */}
                            {adminTab === 'design' && (
                                <div className={`space-y-6 pb-20 animate-fade-in ${isFullScreenDesign ? 'fixed inset-0 z-[600] bg-slate-950 p-0 pt-20 flex' : ''}`}>

                                    {/* BARRA LATERAL FIJA (Solo Screen Completa) - MOVIDA A LA DERECHA */}
                                    {isFullScreenDesign && (
                                        <div className="w-[300px] h-full bg-slate-900 border-l border-white/10 flex flex-col shadow-2xl overflow-y-auto custom-scrollbar shrink-0 order-2">
                                            <div className="p-4 border-b border-white/10 flex items-center justify-center bg-slate-900/50 backdrop-blur-md sticky top-0 z-10 shrink-0">
                                                <h3 className="text-[10px] font-black text-white/40 uppercase tracking-widest flex items-center gap-2">
                                                    <Settings2 size={14} className="text-violet-400" /> Controles Diseño
                                                </h3>
                                            </div>
                                            <div className="p-4 space-y-4 flex-1 overflow-y-auto custom-scrollbar">
                                                {/* Aquí irán los mismos controles que abajo, pero en formato vertical */}
                                            ...
                                            <div className="p-4 border-t border-white/10 bg-slate-900/80 shrink-0">
                                                <div className="flex flex-col gap-1">
                                                    <button
                                                        ...
                                                        className="w-full py-2 bg-violet-600/20 text-violet-300 border border-violet-500/30 rounded-lg font-bold text-[9px] uppercase tracking-widest flex items-center justify-center gap-2 hover:bg-violet-600 hover:text-white transition-all shadow-none"
                                                    >
                                                        <Download size={14} /> Csv v8.4
                                                    </button>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div className={`flex-1 flex flex-col ${isFullScreenDesign ? 'h-full overflow-hidden bg-slate-950 order-1' : ''}`}>
                                        {/* Header Especial para FullScreen */}
                                        {isFullScreenDesign && (
                                            <div className="p-4 px-8 border-b border-white/5 flex items-center justify-between bg-slate-900/30">
                                                <div className="flex items-center gap-4">
                                                    <div className="w-8 h-8 bg-violet-500/20 rounded-lg flex items-center justify-center text-violet-400">
                                                        <Maximize size={18} />
                                                    </div>
                                                    <span className="text-[10px] font-black text-white/40 uppercase tracking-[0.3em]">Modo Pantalla Completa</span>
                                                </div>
                                                <div className="flex items-center gap-4">
                                                    <div className="flex items-center bg-white/5 rounded-xl border border-white/10 p-1">
                                                        <button onClick={() => setCanvasZoom(z => Math.max(0.2, z - 0.1))} className="p-2 hover:bg-white/10 text-white/60 hover:text-white transition-all rounded-lg"><ZoomOut size={16} /></button>
                                                        <span className="text-[10px] font-black text-white px-2 min-w-[50px] text-center">{Math.round(canvasZoom * 100)}%</span>
                                                        <button onClick={() => setCanvasZoom(z => Math.min(3, z + 0.1))} className="p-2 hover:bg-white/10 text-white/60 hover:text-white transition-all rounded-lg"><ZoomIn size={16} /></button>
                                                        <button onClick={() => setCanvasZoom(1)} className="p-2 text-[9px] font-black uppercase tracking-tighter text-violet-400 hover:text-violet-300">Reset</button>
                                                    </div>
                                                </div>
                                            </div>
                                        )}

                                        <div className={`flex-1 flex flex-col items-center justify-center ${isFullScreenDesign ? 'overflow-auto p-4' : 'p-8'}`}>
                                            {/* Preview del Canvas */}
                                            <div className={`card bg-slate-900 border-white/5 overflow-hidden transition-all duration-500 ${isFullScreenDesign ? 'border-none shadow-none bg-transparent w-full flex-1 flex flex-col items-center justify-start' : 'p-8 w-full'}`}>
                                                {!isFullScreenDesign && (
                                                    <div className="flex items-center justify-between mb-8">
                                                        <div>
                                                            <h3 className="text-xl font-black text-white flex items-center gap-3">
                                                                <Eye className="text-violet-400" /> Previsualización Técnica (A3)
                                                            </h3>
                                                            <p className="text-[10px] text-white/40 uppercase tracking-widest mt-1">Escala visual 1:10 • 300 DPI</p>
                                                        </div>
                                                        <button onClick={() => setIsFullScreenDesign(true)} className="p-2.5 bg-white/5 hover:bg-violet-500/20 text-white/60 hover:text-violet-300 rounded-xl transition-all border border-white/10">
                                                            <Maximize size={18} />
                                                        </button>
                                                    </div>
                                                    </div>
                                            ...
                                            <div className="relative bg-white shadow-2xl rounded-sm overflow-hidden"
                                                style={{
                                                    ...
                                                    transform: `scale(${isFullScreenDesign ? canvasZoom * 1.2 : 1})`,
                                                    transformOrigin: 'center center',
                                                    transition: 'transform 0.1s ease-out',
                                                    margin: isFullScreenDesign ? '40px auto' : '0 auto'
                                                }}>
                                                {/* Margen */}
                                                ...
                                                <div className="absolute top-0 w-full flex justify-center gap-[15px] z-20" style={{ top: configOrla.dY / 10 + 'px' }}>
                                                    {[...Array(configOrla.numDocentes)].map((_, i) => (
                                                        <div key={i} className="relative flex flex-col items-center">
                                                            <div className="bg-slate-200 border border-slate-300 rounded-sm relative"
                                                                style={{
                                                            ...
                                                            <div className="mt-[4px] text-center" style={{ width: (configOrla.aW * configOrla.dScale) / 10 + 'px' }}>
                                                                ...
                                                                <p className="text-[5px] mt-[1px] opacity-40 font-bold uppercase">TUTOR/A</p>
                                                            </div>
                                                        </div>
                                                    ))}
                                                </div>
                                                ...
                                                <div className="absolute w-full px-[20px] z-10" style={{ top: configOrla.aStartY / 10 + 'px', padding: `0 ${configOrla.margin / 10}px` }}>
                                                    <div className="grid justify-items-center" style={{
                                                        ...
                                                        {[...Array(configOrla.numAlumnos)].map((_, i) => (
                                                            <div key={i} className="flex flex-col items-center">
                                                                <div className="bg-slate-50 border border-slate-100 rounded-sm relative flex items-center justify-center"
                                                                    style={{ width: configOrla.aW / 10 + 'px', height: configOrla.aH / 10 + 'px' }}>
                                                                    <span className="text-[6px] font-black opacity-[0.05]">{i + 1}</span>
                                                                </div>
                                                                <div className="mt-[3px] text-center" style={{ width: configOrla.aW / 10 + 'px' }}>
                                    ...
                                    {/* Controles (Solo si NO es FullScreen para no duplicar) */}
                                    {!isFullScreenDesign && (
                                    ...
                                    {/* Automatización (Versión compacta) - Solo si NO es FullScreen */}
                                    {!isFullScreenDesign && (
                                        <div className="mt-8 flex items-center justify-between p-4 bg-violet-500/10 border border-violet-500/20 rounded-2xl">
                                            <div className="flex items-center gap-3">
                                                <div className="w-8 h-8 rounded-full bg-violet-500/20 flex items-center justify-center text-violet-400">
                                                    <Terminal size={14} />
                                                </div>
                                                <div>
                                                    <h3 className="text-[10px] font-black text-primary uppercase tracking-widest">Automatización</h3>
                                                    <p className="text-[9px] text-primary/50 uppercase tracking-widest mt-0.5">PS Script v8.4</p>
                                                </div>
                                            </div>
                                            <button
                                                onClick={downloadConfigCSV}
                                                className="px-4 py-2 bg-violet-600 hover:bg-violet-500 text-white rounded-xl font-bold text-[10px] uppercase tracking-widest flex items-center gap-2 transition-all shadow-lg shadow-violet-500/20"
                                            >
